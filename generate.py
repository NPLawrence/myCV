...
def get_pub_latex(context, config):
    ...
    def _get_pub_str(pub, prefix, gidx):
        ...
        if '_note' in pub:
            ...
            note_str = "\color{{{}}}({{{}}})".format("violet", pub['_note'])

            # note_str = f"({pub['_note']})"
        else:
            note_str = ''

        return rf'''\cvline{{{gidx}}}{{{title} {links} \newline {author_str} \newline {yearVenue} {note_str}}}'''
...
# \end{{minipage}}'''
    ...
    def pub_category_details(pubs, details):
        # useful for iterating over different publication categories
    ...
    if 'categories' in config:
        for category in config['categories']:
            pubs = load_and_replace(category['file'])
            heading = str(category['heading'])
            details += rf'\needspace{{2\baselineskip}}\subsection{{{heading}}}'
            details = pub_category_details(pubs, details)
            ...
            if section_tag == 'about':
                ...
                section_template_name = os.path.join(
                    self.SECTIONS_DIR, 'industry' + self._file_ending)
                                 ...
                                 'teaching', 'talks', 'advising', 'references']:
                section_data['items'] = section_content
                section_template_name = os.path.join(
                    self.SECTIONS_DIR, section_tag + self._file_ending)
